...
const PLAN_BG: Record<string, string> = {
  FREE: 'bg-gray-50 dark:bg-gray-900/30',
                  ...
                  {isEditing ? (
                    <div className="space-y-2">
                      <input
                        ...
                        className="w-full rounded-lg border bg-background px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-primary/40"
                      />
                      <input
                        type="text"
                ...
                {isEditing ? (
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="number" min={0} step={0.01} value={editForm.monthlyPrice ?? 0} onChange={(e) => setEditForm((s) => ({ ...s, monthlyPrice: Number(e.target.value) }))} className="mt-1 w-full rounded-md border bg-background px-2 py-1 text-sm" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-muted-foreground">{t('yearlyPrice')}</label>
                      <input type="number" min={0} step={0.01} value={editForm.yearlyPrice ?? 0} onChange={(e) => setEditForm((s) => ({ ...s, yearlyPrice: Number(e.target.value) }))} className="mt-1 w-full rounded-md border bg-background px-2 py-1 text-sm" />
                    </div>
                  </div>
                ) : (
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {([
                  ...
                  <div key={key}>
                    ...
                    {isEditing ? (
                      <input type="number" min={0} value={(editForm as any)[key] ?? 0} onChange={(e) => setEditForm((s) => ({ ...s, [key]: Number(e.target.value) }))} className="mt-1 w-full rounded-md border bg-background px-2 py-1 text-sm" />
                    ) : (
                      <p className="text-sm font-medium mt-0.5">{(plan as any)[key] >= 999999 ? t('unlimited') : (plan as any)[key].toLocaleString()}</p>
                    )}
                  </div>
                ))}
                <div>
                  ...
                  {isEditing ? (
                    <label className="mt-1 flex items-center gap-2 cursor-pointer">
                      ...
                      <span className="text-sm">{editForm.apiAccess ? 'Yes' : 'No'}</span>
                    </label>
                  ) : (
