...
export const MessageBubble = React.memo(function MessageBubble({
  ...
  const mediaUrl =
    msg.contentType !== 'TEXT'
      ? getMediaUrl(msg.content, msg.metadata)
      : undefined;
  ...
  return (
    ...
    >
      <div
        className={cn(
          'relative max-w-[70%] rounded-xl px-4 py-2.5',
          isCustomer
            ? 'rounded-ss-none bg-muted'
            : 'rounded-ee-none bg-primary text-primary-foreground',
        )}
      >
        {/* Copy button */}
        <button
          ...
          className={cn(
            'absolute top-2 end-2 z-10 flex h-6 w-6 items-center justify-center rounded-md opacity-0 transition-opacity group-hover:opacity-70 hover:!opacity-100',
            isCustomer
