...
export default function BillingPage() {
  ...
  const [selectedGateway, setSelectedGateway] = useState<'KASHIER' | 'STRIPE' | 'PAYPAL' | null>(
    null,
  );
  ...
  return (
    <>
      <div>
        <Breadcrumb
          items={[{ label: tb('dashboard'), href: '/dashboard' }, { label: ts('billing') }]}
          className="mb-4"
        />
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="rounded-xl border bg-card p-6 shadow-sm">
              ...
              <div
                className={`grid grid-cols-1 gap-4 ${plans.length === 4 ? 'md:grid-cols-4' : 'md:grid-cols-3'}`}
              >
                {plans.map((plan) => {
                      ...
                      {invoices.map((invoice) => {
                        ...
                        return (
                          <tr key={invoice.id} className="hover:bg-muted/50 transition-colors">
                            <td className="py-3">{fmtDate(invoice.createdAt, locale)}</td>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {t('cancelAtEndDesc', {
                      date: subscription ? fmtDate(subscription.currentPeriodEnd, locale) : '',
                    })}
                  </p>
                </div>
              </button>
