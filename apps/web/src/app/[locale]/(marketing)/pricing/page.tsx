  ...
  return (
    <div className="border-b last:border-b-0">
      <button
        ...
        className="flex w-full items-center justify-between gap-4 py-5 text-start text-sm font-medium hover:text-primary transition-colors"
      >
        <span>{question}</span>
        {open ? (
...
export default function PricingPage() {
  ...
  const faqKeys = ['trial', 'cancel', 'upgrade', 'payment', 'enterprise'] as const;
        ...
        <div className="container max-w-3xl text-center">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href="/register"
              className={cn(
                'inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-3 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors',
              )}
            >
              {t('ctaButton')}
