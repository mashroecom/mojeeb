...
router.get(
  '/real-time',
  validate({ query: realTimeQuerySchema }),
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { orgId, agentIds, startDate, endDate } = req.query as z.infer<
        typeof agentComparisonQuerySchema
      >;
      ...
      const data = await teamPerformanceService.getAgentPerformanceComparison(
        orgId,
        agentIdArray,
        dateRange,
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Generate PDF report
      const pdfBuffer = await pdfReportService.generateTeamPerformanceReport(
        orgId,
        startDate,
        endDate,
      );
