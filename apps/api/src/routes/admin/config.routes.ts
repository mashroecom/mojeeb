...
const CONFIG_DEFAULTS: Record<
  string,
  { category: string; label: string; description: string; isSecret: boolean; defaultValue?: string }
> = {
  // AI Providers
  OPENAI_API_KEY: {
    category: 'ai',
  ...
  KASHIER_MERCHANT_ID: {
    ...
    description: 'Merchant identifier for Kashier payment gateway',
    isSecret: false,
  },
  KASHIER_API_KEY: {
    category: 'payment',
    label: 'Kashier API Key',
    description: 'API key for Kashier payment processing',
    isSecret: true,
  },
  KASHIER_WEBHOOK_SECRET: {
    category: 'payment',
    label: 'Kashier Webhook Secret',
    description: 'Secret for verifying Kashier webhook signatures',
    isSecret: true,
  },
  // Email
  RESEND_API_KEY: {
  ...
  EMAIL_FROM: {
    category: 'email',
    label: 'From Address',
    description: 'Default sender email address (e.g. noreply@mojeeb.app)',
    isSecret: false,
  },
  SALES_EMAIL: {
    category: 'email',
    label: 'Sales Email',
    description: 'Sales team email for contact forms',
    isSecret: false,
  },
  SUPPORT_EMAIL: {
    category: 'email',
    label: 'Support Email',
    description: 'Support team email for help requests',
    isSecret: false,
  },
  // Meta Platform
  META_APP_SECRET: {
    ...
    description: 'App secret for Facebook/Meta platform',
    isSecret: true,
  },
  META_VERIFY_TOKEN: {
    category: 'meta',
    label: 'Webhook Verify Token',
    description: 'Token for verifying Meta webhook subscriptions',
    isSecret: false,
  },
  ...
  WHATSAPP_PHONE_NUMBER_ID: {
    ...
    description: 'Phone number ID for WhatsApp Business',
    isSecret: false,
  },
  MESSENGER_PAGE_ACCESS_TOKEN: {
    category: 'meta',
    label: 'Messenger Page Token',
    description: 'Page access token for Facebook Messenger',
    isSecret: true,
  },
  // OAuth
  GOOGLE_CLIENT_ID: {
  ...
  APP_URL: {
    ...
    description: 'Public URL of the application (e.g. https://mojeeb.app)',
    isSecret: false,
  },
  API_URL: {
    category: 'general',
    label: 'API URL',
    description: 'Public URL of the API server (e.g. https://api.mojeeb.app)',
    isSecret: false,
  },
  JWT_SECRET: {
    category: 'general',
  ...
  NOTIFY_NEW_ORG: {
    ...
    description: 'Notify when a new organization is created',
    isSecret: false,
    defaultValue: 'true',
  },
  NOTIFY_FAILED_PAYMENT: {
    ...
    description: 'Notify on failed payment attempts',
    isSecret: false,
    defaultValue: 'true',
  },
  ...
  NOTIFY_NEW_DEMO: {
    ...
    description: 'Notify on new demo requests',
    isSecret: false,
    defaultValue: 'true',
  },
  DAILY_SUMMARY: {
    category: 'notifications',
    label: 'Daily Summary',
    description: 'Send a daily summary email to admin',
    isSecret: false,
    defaultValue: 'false',
  },
  ADMIN_ALERT_EMAIL: {
...
async function testCategory(category: string): Promise<CategoryTestResult> {
  switch (category) {
    ...
    default:
      return {
        success: true,
        message: 'No connectivity test available for this category',
        results: [],
      };
  }
}
...
async function testAi(): Promise<CategoryTestResult> {
    ...
    if (!openaiKey) {
      ...
      if (response.ok) {
        results.push({
          ...
          label: 'OpenAI API Key',
          success: true,
          message: 'Valid - connected successfully',
        });
      } else {
        results.push({
          key: 'OPENAI_API_KEY',
          label: 'OpenAI API Key',
          success: false,
          message: `Invalid (status ${response.status})`,
        });
      }
    }
  } catch (err) {
  ...
  return {
    success: anySuccess,
    message: allSuccess
      ? 'All AI providers are valid'
      : anySuccess
        ? 'Some AI providers need attention'
...
async function testEmail(): Promise<CategoryTestResult> {
  ...
  return {
    ...
    message: results.every((r) => r.success)
      ...
      : 'Some email settings need attention',
    results,
  };
}
...
async function testPayment(): Promise<CategoryTestResult> {
  ...
  return {
    success: results.every((r) => r.success),
    message: results.every((r) => r.success)
      ? 'Payment credentials are valid'
      : 'Some payment settings need attention',
    results,
  };
}
...
async function testMeta(): Promise<CategoryTestResult> {
    ...
    if (!whatsappToken) {
      ...
      if (phoneId) {
        ...
        results.push({
          ...
          label: 'WhatsApp Access Token',
          success: false,
          message: 'Phone Number ID needed to test',
        });
      }
    }
  } catch (err) {
  ...
  try {
    ...
    if (!messengerToken) {
      ...
      const response = await fetch(
        `https://graph.facebook.com/v18.0/me?access_token=${messengerToken}`,
      );
      results.push({
        key: 'MESSENGER_PAGE_ACCESS_TOKEN',
        label: 'Messenger Page Token',
  ...
  return {
    success: results.every((r) => r.success),
    message: results.every((r) => r.success)
      ? 'All Meta integrations are valid'
      : anySuccess
        ? 'Some Meta tokens need attention'
        : 'No Meta tokens configured',
    results,
  };
}
...
async function testOAuth(): Promise<CategoryTestResult> {
  ...
  results.push({
    ...
    message: !clientId
      ? 'Not configured'
      : googleClientIdPattern.test(clientId)
        ? 'Format valid'
        : 'Invalid format (expected *.apps.googleusercontent.com)',
  });
  ...
  return {
    ...
    message: results.every((r) => r.success)
      ...
      : 'Some OAuth settings need attention',
    results,
  };
}
...
async function testGeneral(): Promise<CategoryTestResult> {
  ...
  results.push({
    ...
    message: !jwtSecret
      ? 'Not configured'
      : jwtSecret.length < 32
        ? `Too short (${jwtSecret.length} chars, min 32)`
        : 'Configured (secure length)',
  });
  ...
  return {
    ...
    message: results.every((r) => r.success)
      ...
      : 'Some general settings need attention',
    results,
  };
}
