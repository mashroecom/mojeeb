...
router.get(
  '/growth',
  validate({ query: growthQuerySchema }),
  ...
  try {
    ...
    const [
      ...
      prisma.subscription.count({
        where: { status: 'ACTIVE', plan: { not: 'FREE' }, createdAt: { lte: now } },
      }),
      prisma.subscription.count({
        where: { status: 'ACTIVE', plan: { not: 'FREE' }, createdAt: { lte: currentStart } },
      }),
      prisma.invoice.aggregate({
        where: { status: 'PAID', paidAt: { gte: currentStart, lte: now } },
        _sum: { amount: true },
      }),
      prisma.invoice.aggregate({
        where: { status: 'PAID', paidAt: { gte: previousStart, lte: currentStart } },
        _sum: { amount: true },
      }),
    ...
    const metrics = [
      {
        ...
        changePercent: changePercent(currentUsers, previousUsers),
      },
      {
        label: 'activeSubscriptions',
        value: currentActiveSubs,
        previousValue: previousActiveSubs,
        changePercent: changePercent(currentActiveSubs, previousActiveSubs),
      },
      {
        label: 'monthlyRevenue',
        value: mrrCurrent,
        previousValue: mrrPrevious,
        changePercent: changePercent(mrrCurrent, mrrPrevious),
      },
      {
      ...
      },
      {
        ...
        value: tokensCurrent,
        previousValue: tokensPrevious,
        changePercent: changePercent(tokensCurrent, tokensPrevious),
      },
      {
        label: 'totalCost',
        value: costCurrent,
        previousValue: costPrevious,
        changePercent: changePercent(costCurrent, costPrevious),
      },
...
router.get(
  '/sparkline',
  validate({ query: sparklineSchema }),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        switch (metric) {
          ...
          case 'conversations':
            ...
            break;
          case 'tokens': {
            const tok = await prisma.tokenUsage.aggregate({
              where: { createdAt: { gte: dayStart, lte: dayEnd } },
              _sum: { totalTokens: true },
            });
...
router.get(
  ...
  async (_req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const rows = await prisma.$queryRaw<{ dayOfWeek: number; hour: number; count: bigint }[]>`
    ...
    const [recentUsers, recentInvoices, recentErrors] = await Promise.all([
      ...
      prisma.invoice.findMany({
        ...
        select: {
          ...
          currency: true,
          paidAt: true,
          subscription: { select: { org: { select: { name: true } } } },
        },
        orderBy: { paidAt: 'desc' },
        take: limit,
      }),
