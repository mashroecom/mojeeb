...
router.post('/:tagId/conversations/:conversationId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const orgId = req.params.orgId as string;
    const tagId = req.params.tagId as string;
    ...
    const [tag, conversation] = await Promise.all([
      ...
      prisma.conversation.findFirst({ where: { id: conversationId, orgId }, select: { id: true } }),
    ]);
    if (!tag) return res.status(404).json({ success: false, error: 'Tag not found' });
...
router.delete('/:tagId/conversations/:conversationId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const orgId = req.params.orgId as string;
    const tagId = req.params.tagId as string;
    const conversationId = req.params.conversationId as string;
    ...
    if (!tag || !conversation) return res.status(404).json({ success: false, error: 'Not found' });
