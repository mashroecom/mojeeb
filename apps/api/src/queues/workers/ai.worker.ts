    ...
    try {
      ...
      if (convCheck?.status === 'HANDED_OFF') {
        logger.info(
          ...
          'Skipping AI — conversation already HANDED_OFF',
        );
        return;
      }
      ...
      if (isHandoff) {
        ...
        try {
          const notifTitle =
            agent.language === 'ar' ? 'محادثة تحتاج تدخل بشري' : 'Conversation needs human agent';
          const notifBody =
            agent.language === 'ar'
      ...
      if (updatedConversation.messageCount % 3 === 0) {
        try {
          ...
          logger.warn(
            ...
            'Enhanced summary generation failed',
          );
        }
      }
      ...
      if (result.extractedData) {
        try {
          ...
          logger.warn(
            { err, conversationId: data.conversationId },
            'Customer data extraction save failed',
          );
        }
      }
      ...
      if (result.lead) {
        try {
          ...
          logger.warn(
            { err: leadErr, conversationId: data.conversationId },
            'Lead extraction save failed',
          );
        }
      }
      ...
      try {
        ...
        const fallback =
          org?.defaultLanguage === 'ar' ? org?.fallbackMessageAr : org?.fallbackMessage;
        if (fallback) {
