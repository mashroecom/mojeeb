...
export class AnalyticsService {
  ...
  async getAgentPerformance(orgId: string) {
    return this.cached(`agent-perf:${orgId}`, 300, async () => {
      const [agentConversations, agentResolvedConversations, agentMessages, agentResponseTimes] =
        await Promise.all([
      ...
      const agentIds = agentConversations
        .map((c) => c.agentId)
      ...
      return Array.from(agentStats.entries()).map(([agentId, stats]) => ({
        ...
        avgResponseTimeMs:
          stats.latencyCount > 0 ? Math.round(stats.totalLatency / stats.latencyCount) : 0,
  ...
  async getLeadFunnel(orgId: string) {
    return this.cached(`lead-funnel:${orgId}`, 300, async () => {
      ...
      const conversionRate =
        totalLeads > 0 ? Math.round((convertedCount / totalLeads) * 10000) / 10000 : 0;
  ...
  async getCsatTrends(orgId: string, params: ConversationMetricsParams) {
    ...
    return this.cached(cacheKey, 300, async () => {
      const rows = await prisma.$queryRaw<{ date: Date; avgRating: number; count: bigint }[]>`
  ...
  async getQueueDepth(orgId: string, userId?: string) {
    const cacheKey = userId ? `queue-depth:${orgId}:${userId}` : `queue-depth:${orgId}`;
    ...
    return this.cached(cacheKey, 60, async () => {
      const where = userId
        ? {
            ...
            status: { in: ['ACTIVE' as const, 'HANDED_OFF' as const, 'WAITING' as const] },
          }
        : {
            orgId,
            status: { in: ['ACTIVE' as const, 'HANDED_OFF' as const, 'WAITING' as const] },
  ...
  async getTeamPerformance(orgId: string, params: ConversationMetricsParams) {
    ...
    `;
      ...
      const activeAgentsByPeriod = await prisma.$queryRaw<{ date: Date; count: bigint }[]>`
      SELECT ${Prisma.raw(trunc.replace(/"createdAt"/g, 'm."createdAt"'))} AS "date",
